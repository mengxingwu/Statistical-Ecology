...
FishCopPermute<-myPermutPairs(CopDens, FishDens, 5000)
FishPermute<-FishCopPermute[[2]]
CopPermute<-FishCopPermute[[1]]
#paste('Let us check the randomized fish dataset')
#head(FishPermute)
#paste('Let us check the randomized copepod dataset')
#head(CopPermute)
beta1_Permute<-c()
for(i in 1:5000){
  
  beta1_Permute[i]<-MylmCoefficient(as.vector(as.matrix(CopPermute[i])), 
                                    as.vector(as.matrix(FishPermute[i])))[1]
  
}
paste('Check the distribution of randomized beta_1')
hist(beta1_Permute, breaks=100, main=expression(paste('Permutation of ', beta, '1')), xlab=expression(paste(beta, '1')), las=1)
```

Question_1: Compute P value and examine significance
---
```{r p value.1, eval=FALSE}
Beta1<-MylmCoefficient(CopDens, FishDens)[1]
PvalueBeta1<-1-myECDF(beta1_Permute, Beta1)

paste('P value=', PvalueBeta1)
paste('P>0.05')
paste('No significant difference from null')
